...
// add body class theo ngôn ngữ
// add_filter('body_class','caia_add_body_class_language');
// function caia_add_body_class_language($classes){
...
genesis_register_sidebar( 
	array(
		'id'			=> 'content-slider',
		'name'			=> 'Trang chủ - Nội dung slider',
	)
);
...
function caia_add_contactus(){
	if( is_active_sidebar( 'nhantuvan' ) ){
		echo '<div class="nhantuvan section">';
...
function adsdigi_enqueue_styles() {
    wp_enqueue_style('adsdigi-style', get_template_directory_uri() . '/style.css');
}
add_action('wp_enqueue_scripts', 'adsdigi_enqueue_styles');
